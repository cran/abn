                             ...
                             ## only compute option here is C since fast and INLA slower and less reliable
                               if(force.method=="notset" || force.method=="C"){
                                 ...
                                 r <- try(res.c <- .Call("fit_single_node",
                                                ...
                                                as.integer(child),
                                                ## childnode
                                                as.integer(dag.m[child,]),
                                                ## parent combination
                                                as.integer(dim(dag.m)[1]),
                                                ## number of nodes/variables
                                                as.integer(var.types),
                                                ## type of densities
                                                as.integer(sum(dag.m[child,])),
                                                ## max.parents
                                                ...
                                                as.integer(grouped.vars-1),
                                                ## int.vector of variables which are mixed model nodes -1 for C
                                                group.ids,
                                                ## group memberships - note indexed from 1
                                                ...
                                                as.integer(0), 
                                                ## modes only - false here as only applies to glmms
                                                ...
                                                ## Not applicable 
                                  PACKAGE="abn" 
                                  ## uncomment to load as package not shlib
                                                 ))
                          if(length(attr(r,"class")>0) && attr(r,"class")=="try-error"){ if (verbose) cat("## !!! Laplace approximation failed\n")
                           ...
                           ########################################################### 
                                  ...
                                  res.inla <- calc.node.inla.glmm(child,
                                                            ...
                                                            ## exposure 
                                                            TRUE,
                                                            ## always compute marginals - since only way to check results
