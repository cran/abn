...
search.hillclimber <- function(score.cache = NULL, num.searches = 1, seed = 0, verbose = FALSE, timing.on = TRUE, start.dag = NULL, support.threshold = 0.5, create.graph = FALSE, dag.retained = NULL) {
message("Best practice with abn 2.0 requires to use the function searchHillclimber().")    
    searchHillclimber(score.cache, num.searches , seed , verbose , timing.on, start.dag , support.threshold, create.graph, dag.retained)
}


searchHillclimber <- function(score.cache = NULL, num.searches = 1, seed = 0, verbose = FALSE, timing.on = TRUE, start.dag = NULL, support.threshold = 0.5, create.graph = FALSE, dag.retained = NULL) {
    ...
    if (create.graph) {
        ...
        out <- list(init.score = init.scores, final.score = fin.scores, init.dag = init.mat, final.dag = fin.mat, consensus = (con.dag.binary), support.threshold = support.threshold, graph = mygraph, score.cache = score.cache)
        class(out) <- c("abnHillClimber", "abnLearned")
        return(out)
    } else {
        out <- list(init.score = init.scores, final.score = fin.scores, init.dag = init.mat, final.dag = fin.mat, consensus = (con.dag.binary), support.threshold = support.threshold, score.cache = score.cache)
        class(out) <- c("abnHillClimber", "abnLearned")
        return(out)
    }
