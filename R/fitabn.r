...
fitabn <- function(data.df,dag.m,hyper.params=list("mean"=c(0),"sd"=c(sqrt(1000)),"shape"=c(0.001),"scale"=c(1/0.001)),max.iters=100,epsabs=1e-7, 
                   verbose=FALSE, error.verbose=FALSE) {
    ...
    ## call to C
    res <- .Call("fitnetwork_additive",obsdata,dag,prior.mean,prior.sd,prior.gamma.shape,prior.gamma.scale,maxparents,loc.verbose,var.types,max.iters,epsabs,
                                        loc.error.verbose
